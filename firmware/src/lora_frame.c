...
#define LORA_FRAME_MSG_OVERHEAD                                                \
    (LORA_FRAME_SEND_ID_OVERHEAD + LORA_FRAME_SEQ_CNT_OVERHEAD +               \
     LORA_FRAME_SECONDS_OVERHEAD + LORA_FRAME_NS_OVERHEAD)
#define LORA_FRAME_OVERHEAD       (LORA_FRAME_MSG_OVERHEAD + LORA_FRAME_CRC_OVERHEAD)

#define LORA_FRAME_SEND_ID_OFFSET 0
#define LORA_FRAME_SEQ_CNT_OFFSET                                              \
    (LORA_FRAME_SEND_ID_OFFSET + LORA_FRAME_SEND_ID_OVERHEAD)
#define LORA_FRAME_SECONDS_OFFSET                                              \
    (LORA_FRAME_SEQ_CNT_OFFSET + LORA_FRAME_SEQ_CNT_OVERHEAD)
#define LORA_FRAME_NS_OFFSET                                                   \
    (LORA_FRAME_SECONDS_OFFSET + LORA_FRAME_SECONDS_OVERHEAD)
#define LORA_FRAME_CRC_OFFSET (LORA_FRAME_NS_OFFSET + LORA_FRAME_NS_OVERHEAD)

const size_t lora_frame_size = LORA_FRAME_OVERHEAD;

static void serialize_field(uint8_t *buf, const uint64_t field,
                            size_t field_size) {
    size_t i = 0;
    for (; i < field_size; i++) {
...
static void deserialize_field(const uint8_t *buf, void *field,
                              size_t field_size) {
...
#define DESERIALIZE(buf_, field_)                                              \
    deserialize_field(buf_, &field_, sizeof(field_))
...
static uint16_t compute_crc(const uint8_t *buf, size_t len) {
    ...
    ret = crc16(CONFIG_LORA_CRC_POLYNOMIAL, CONFIG_LORA_CRC_SEED,
                reflected_data, len);
#else
